...
def search_page():
     if request.method == 'POST':
        ...
        # else:
        #     s= 14
        data=pd.DataFrame(df[df[search] == str(detail)])
...
@app.route('/oldpdf',methods=['GET', 'POST'])
def oldpdf():
        pdf = FPDF('P', 'mm', 'A4')
        pdf.add_page()
        pdf.set_margins(0, 0, 0)
        ...
        return redirect(url_for('pdfview'))
        # return render_template('result.html')
        # return render_template('pdfview.html',data=data)
